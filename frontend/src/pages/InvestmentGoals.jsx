...
function ProgressBar({ current, target }) {
  ...
  const color = pct >= 100 ? "#4ade80" : pct >= 60 ? "#C9A227" : "#818cf8";
  return (
    <div>
    ...
    <div style={{ minHeight: "100vh", background: "#020617", color: "#e2e8f0" }}>
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "24px 0 32px", flexWrap: "wrap", gap: 16 }}>
          <div>
            <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 32, marginBottom: 6 }}>🎯 Investment Goals</h1>
            <p style={{ color: "#64748b", fontSize: 14 }}>Define and track your wine investment objectives</p>
          </div>
          <button onClick={() => setShowAdd(true)} style={{ background: "#C9A227", color: "#020617", border: "none", borderRadius: 10, padding: "10px 20px", fontWeight: 700, cursor: "pointer" }}>
            + New Goal
          </button>
        </div>
        ...
        {!loading && goals.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 24px", background: "rgba(11,18,32,0.7)", border: "1px dashed rgba(30,41,59,0.6)", borderRadius: 16 }}>
            ...
            <button onClick={() => setShowAdd(true)} style={{ background: "#C9A227", color: "#020617", border: "none", borderRadius: 10, padding: "12px 24px", fontWeight: 700, cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          {goals.map(g => {
            ...
            return (
              <div key={g.id} style={{ background: "rgba(11,18,32,0.85)", border: `1px solid ${isCompleted ? "rgba(74,222,128,0.3)" : isAtRisk ? "rgba(248,113,113,0.3)" : "rgba(30,41,59,0.5)"}`, borderRadius: 16, padding: "22px 24px" }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16, gap: 12 }}>
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 18 }}>{g.title}</h3>
                      {isCompleted && <span style={{ fontSize: 11, color: "#4ade80", background: "rgba(74,222,128,0.1)", padding: "2px 8px", borderRadius: 4 }}>✓ Completed</span>}
                      {isAtRisk && !isCompleted && <span style={{ fontSize: 11, color: "#f87171", background: "rgba(248,113,113,0.1)", padding: "2px 8px", borderRadius: 4 }}>⚠️ At risk</span>}
                    </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12, marginTop: 18 }}>
                  {[
                    ...
                    { label: "Monthly Needed", value: `€${Math.round(g.monthly_needed || 0).toLocaleString()}`, color: "#C9A227" },
                    ...
                    <div key={s.label} style={{ background: "rgba(30,41,59,0.3)", borderRadius: 8, padding: "10px 12px" }}>
                      <div style={{ fontSize: 10, color: "#64748b" }}>{s.label}</div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: s.color || "#e2e8f0", marginTop: 2 }}>{s.value}</div>
                    </div>
                  ))}
                </div>

                {!isCompleted && (
                  <div style={{ marginTop: 14, padding: "10px 14px", background: "rgba(129,140,248,0.08)", border: "1px solid rgba(129,140,248,0.15)", borderRadius: 8, fontSize: 13, color: "#94a3b8" }}>
                    ...
                    Based on 8% average annual wine market return. <span style={{ color: "#475569" }}>*Estimate only — not guaranteed.</span>
                  </div>
                )}
              </div>
            );
        ...
        {showAdd && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 9000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={() => setShowAdd(false)}>
            <div style={{ background: "#0f172a", border: "1px solid rgba(201,162,39,0.3)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 460, maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, marginBottom: 24 }}>New Investment Goal</h3>
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ fontSize: 12, color: "#94a3b8", display: "block", marginBottom: 8 }}>Strategy</label>
                <div style={{ display: "flex", gap: 8 }}>
                  {["conservative", "balanced", "aggressive"].map(s => (
                    <button key={s} onClick={() => setForm(p => ({ ...p, strategy: s }))} style={{
                      ...
                      color: form.strategy === s ? "#C9A227" : "#94a3b8",
              ...
              {preview && (
                <div style={{ background: "rgba(201,162,39,0.08)", border: "1px solid rgba(201,162,39,0.2)", borderRadius: 10, padding: "14px 16px", marginBottom: 20 }}>
                  <div style={{ fontSize: 13, color: "#C9A227", fontWeight: 700, marginBottom: 4 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setShowAdd(false)} style={{ flex: 1, padding: 10, background: "rgba(30,41,59,0.5)", color: "#94a3b8", border: "1px solid rgba(30,41,59,0.6)", borderRadius: 8, cursor: "pointer" }}>Cancel</button>
                <button onClick={createGoal} disabled={!form.target_amount || !form.target_date} style={{ flex: 2, padding: 10, background: "#C9A227", color: "#020617", border: "none", borderRadius: 8, fontWeight: 700, cursor: "pointer" }}>Create Goal</button>
              </div>
            </div>
          </div>
        )}
