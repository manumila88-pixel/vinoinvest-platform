...
const OCCASIONS = ["Cena", "Celebrazione", "Degustazione", "Regalo", "Business", "Casual", "Club del vino", "Altro"];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--vi-bg)", color: "var(--vi-text)" }}>
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "40px 24px" }}>
        <a href="/" style={{ color: "var(--vi-text-dim)", fontSize: 13, textDecoration: "none" }}>← Indietro</a>

        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "24px 0 32px", flexWrap: "wrap", gap: 16 }}>
          <div>
            <h1 style={{ fontFamily: "var(--vi-font-display)", fontSize: 32, marginBottom: 6 }}>Diario del Vino</h1>
            <p style={{ color: "var(--vi-text-dim)", fontSize: 14 }}>{entries.length} {entries.length === 1 ? "nota" : "note"} di degustazione</p>
          </div>
          <button
        ...
        {!loading && !fetchError && filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 24px", color: "var(--vi-text-dim)" }}>
            ...
            <p style={{ fontSize: 13 }}>Inizia a registrare le tue esperienze enologiche</p>
          </div>
        )}
        ...
        {showAdd && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 9000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={() => setShowAdd(false)}>
            <div style={{ background: "var(--vi-bg-elev)", border: "1px solid rgba(201,162,39,0.3)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
              <h3 style={{ fontFamily: "var(--vi-font-display)", fontSize: 20, marginBottom: 20 }}>Nuova Nota di Degustazione</h3>

              {[
                { label: "Nome del Vino", key: "wine_name", type: "text", placeholder: "es. Barolo Cascina Francia" },
                { label: "Annata", key: "vintage", type: "text", placeholder: "es. 2016" },
                { label: "Data di Degustazione", key: "tasted_at", type: "date" },
                { label: "Con Chi", key: "companions", type: "text", placeholder: "es. Cena in famiglia" },
              ].map(f => (
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={{ fontSize: 12, color: "var(--vi-text-dim)", display: "block", marginBottom: 8 }}>Valutazione</label>
                <StarRating value={form.rating} onChange={r => setForm(p => ({ ...p, rating: r }))} />
              </div>

              <div style={{ marginBottom: 14 }}>
                <label style={{ fontSize: 12, color: "var(--vi-text-dim)", display: "block", marginBottom: 6 }}>Occasione</label>
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ fontSize: 12, color: "var(--vi-text-dim)", display: "block", marginBottom: 6 }}>Note di Degustazione</label>
                <textarea
                  placeholder="Descrivi gli aromi, i sapori, la texture..."
                  value={form.notes}
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setShowAdd(false)} style={{ flex: 1, padding: "10px", background: "rgba(30,41,59,0.5)", color: "var(--vi-text-dim)", border: "1px solid var(--vi-border)", borderRadius: 8, cursor: "pointer" }}>Annulla</button>
                <button onClick={saveEntry} style={{ flex: 2, padding: "10px", background: "var(--vi-accent)", color: "var(--vi-bg)", border: "none", borderRadius: 8, fontWeight: 700, cursor: "pointer" }}>Salva Nota</button>
              </div>
            </div>
          </div>
        )}
