...
const FALLBACK_NEWS = [
  ...
  { id: 6, title: "Fine Wine Outperforms Gold in Volatile Markets", description: "New research from Knight Frank's Luxury Investment Index shows fine wine delivering 12.4% annual returns over 10 years, outperforming traditional safe-haven assets.", source: { name: "Financial Times" }, publishedAt: "2026-05-29T08:30:00Z", url: "#", country: "UK", category: "investment" },
  ...
  { id: 10, title: "Penfolds Grange 2023 Sells Out in 48 Hours Across Europe", description: "The release of Penfolds Grange 2023 has been met with extraordinary demand from European collectors, with the allocation selling out within 48 hours.", source: { name: "Wine Spectator" }, publishedAt: "2026-05-25T14:00:00Z", url: "#", country: "AU", category: "market" },
  { id: 11, title: "Romanée-Conti 2022: Most Anticipated DRC Release Since 2015", description: "DRC's 2022 vintage is being positioned as one of the decade's greatest, with négociants allocating strict limits and secondary market prices at €28,000+ per bottle.", source: { name: "Burghound" }, publishedAt: "2026-05-24T11:00:00Z", url: "#", country: "FR", category: "market" },
  { id: 12, title: "South African Fine Wine: Swartland Revolution Attracts International Capital", description: "Premium South African producers like Sadie Family are seeing significant price appreciation as international collectors discover the region's investment potential.", source: { name: "Platter's Wine Guide" }, publishedAt: "2026-05-23T10:00:00Z", url: "#", country: "ZA", category: "market" },
  { id: 13, title: "Digital Wine Trading Platforms Record €320M Volume in Q1 2026", description: "Online fine wine trading platforms report a combined volume of €320 million in Q1 2026, driven by younger investors seeking portfolio diversification.", source: { name: "Decanter Business" }, publishedAt: "2026-05-22T09:00:00Z", url: "#", country: "UK", category: "investment" },
  ...
  { id: 15, title: "Climate Change Shifts Fine Wine Investment Profiles Northward", description: "Wine investors are reassessing regional risk profiles as climate change shifts production patterns, with northern regions benefiting.", source: { name: "The Economist" }, publishedAt: "2026-05-20T08:00:00Z", url: "#", country: "UK", category: "investment" },
  { id: 16, title: "Christie's May Wine Auction Totals €12M, 23% Above Estimate", description: "Christie's international wine auction achieved €12 million in total sales with bidders from 42 countries, reflecting robust collector demand.", source: { name: "Christie's" }, publishedAt: "2026-05-19T16:00:00Z", url: "#", country: "UK", category: "auction" },
  { id: 17, title: "Vega Sicilia Único 2018 Sets New Spanish Wine Price Record", description: "The latest release of Vega Sicilia Único has been allocated at €450 per bottle, marking a 35% increase from the previous vintage.", source: { name: "Decanter Spain" }, publishedAt: "2026-05-18T11:00:00Z", url: "#", country: "ES", category: "market" },
  ...
  { id: 20, title: "IoT Smart Cellars Revolutionize €1M+ Private Wine Collections", description: "New IoT-enabled wine storage systems are helping collectors track provenance, optimal drinking windows, and real-time market values across portfolios.", source: { name: "Forbes" }, publishedAt: "2026-05-15T10:00:00Z", url: "#", country: "US", category: "technology" },
];
...
// NewsAPI as tertiary fallback (only if key provided)
...
let newsApiCache = null;
let newsApiCacheTime = 0;
const NEWS_API_TTL = 30 * 60 * 1000; // 30min

router.get("/", async (req, res) => {
  const { country = "all", category = "all", lang } = req.query;

  // 1. Try RSS feeds (free, no API key)
  let articles = null;
  let dataSource = "rss";
  ...
  try {
    ...
    console.log("[news] RSS fetch error:", e.message);
  }

  // 2. Try NewsAPI if key available and RSS failed
  if (!articles && process.env.NEWS_API_KEY) {
    if (!newsApiCache || Date.now() - newsApiCacheTime > NEWS_API_TTL) {
      try {
        newsApiCache = await fetchFromNewsAPI(process.env.NEWS_API_KEY);
        newsApiCacheTime = Date.now();
        dataSource = "newsapi";
      } catch (e) {
        console.log("[news] NewsAPI unavailable:", e.message);
      }
    }
    if (newsApiCache) { articles = newsApiCache; dataSource = "newsapi"; }
  }
  ...
  if (country !== "all") {
    const filtered = result.filter(a => (a.country || "UK").toUpperCase() === country.toUpperCase());
    if (filtered.length > 0) result = filtered;
  }
  if (category !== "all") {
    const filtered = result.filter(a => a.category === category);
    if (filtered.length > 0) result = filtered;
  }
  ...
  if (targetLang && targetLang !== "en") {
    try {
      result = await translateObjects(result, ["title", "description"], targetLang, "en");
    } catch (e) {
      console.warn("[news] Translation failed:", e.message);
    }
  }

  res.json({ articles: result, total: result.length, source: dataSource });
});
